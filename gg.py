...
#     def _process_virtual_trajectories(self, detections, img):
#         """Process virtual trajectories for lost tracks."""
#         if len(detections) > 0:
#             det_features = [det.feature for det in detections]
            
#             # Update lost tracks if they match with current detections
#             lost_track_ids = list(self.lost_tracks.keys())
#             for track_id in lost_track_ids:
#                 lost_info = self.lost_tracks[track_id]
#                 last_frame = lost_info['last_frame']
#                 time_gap = self.frame_idx - last_frame
...
#                     z2 = detections[best_match].to_xysr()
#                     virtual_boxes = self._interpolate_boxes(z1, z2, time_gap)
                    
#                     # Apply virtual boxes
#                     track = lost_info['track']
#                     conf = self.virtual_conf * (1 + best_sim) / 2
                    
#                     for box in virtual_boxes:
#                         track.virtual_trajectory.append((box, conf))
...
#         """Update with virtual trajectory support.
...
#         # Process detections and get features
#         dets_processed = self.preprocess_dets(dets)
#         if len(dets_processed) > 0:
#             self._process_virtual_trajectories(dets_processed, img)
        
#         return super().update(dets_processed, img)
...
#     """Lớp thực hiện phát hiện và theo dõi đối tượng trong video."""
    
#     def __init__(self, model_weights, capture_path, output_dir, min_temporal_threshold=0,
#                  max_temporal_threshold=0, iou_threshold=0.2):
...
#         """Khởi tạo Enhanced StrongSort với hỗ trợ quỹ đạo ảo."""
#         reid_weights = Path("osnet_dcn_x0_5_endocv.pt")
#         return EndoStrongSort(
#             reid_weights,
...
#         if not cap.isOpened():
#             raise IOError(f"Error opening video file {self.capture_path}")
...
#         """Vẽ các track lên frame và ghi vào file txt."""
...
#         # Convert tracks to array if it's not already
#         if not isinstance(tracks, np.ndarray):
#             tracks = np.array(tracks)
            
#         for track in tracks:
#             if len(track) >= 8:  # Format from StrongSort: [x1, y1, x2, y2, track_id, conf, cls, det_ind]
...
#             else:
#                 continue  # Skip if track format is invalid
...
#     parser.add_argument("--video_dir", type=str, default="video_test_x", help="Input video directory")
#     parser.add_argument("--model_dir", type=str, default="model_yolo", help="YOLO model directory")
#     parser.add_argument("--output_dir", type=str, default="content/runs_3vids_xysr_vt", help="Output directory")
#     parser.add_argument("--min_temporal_threshold", type=float, default=0, help="Minimum temporal threshold")
#     parser.add_argument("--max_temporal_threshold", type=float, default=0, help="Maximum temporal threshold")
#     parser.add_argument("--iou_threshold", type=float, default=0.2, help="IoU threshold for ID update")
...
#                 # Create subdirectory for each model's results
#                 model_output_dir = output_dir / model_path.stem
#                 model_output_dir.mkdir(parents=True, exist_ok=True)
                
#                 print(f"\nProcessing {video_path.name} with {model_path.name}")
#                 detector = ObjectDetection(
#                     model_weights=str(model_path),
#                     capture_path=str(video_path),
...
class EndoStrongSort(StrongSortXYSR):
    ...
    def _process_virtual_trajectories(self, detections, img):
        ...
        if len(detections) > 0:
            det_features = self.model.get_features(detections[:, :4], img)
...
class ObjectDetection:
    ...
    def _draw_tracks(self, frame, tracks, txt_file):
        ...
        for track in tracks:
            ...
            # # In thông tin box dự đoán và box thực tế
            # predicted_state = self.tracker.tracked_stracks[0].mean if self.tracker.tracked_stracks else None
            # if predicted_state is not None:
            #     pred_x1 = predicted_state[0] - predicted_state[3] * predicted_state[2] / 2
            #     pred_y1 = predicted_state[1] - predicted_state[3] / 2
            #     pred_x2 = predicted_state[0] + predicted_state[3] * predicted_state[2] / 2
            #     pred_y2 = predicted_state[1] + predicted_state[3] / 2
            #     print(f"ID {id_} - Frame {frame_id}:")
            #     print(f"Predicted box: ({pred_x1:.1f}, {pred_y1:.1f}, {pred_x2:.1f}, {pred_y2:.1f})")
            #     print(f"Actual box:    ({x1}, {y1}, {x2}, {y2})")
            #     print(f"Difference:    ({abs(pred_x1-x1):.1f}, {abs(pred_y1-y1):.1f}, {abs(pred_x2-x2):.1f}, {abs(pred_y2-y2):.1f})\n")
